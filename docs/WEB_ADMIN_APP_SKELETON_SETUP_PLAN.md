...
目標是讓下一階段能小步建立 Next.js / React / TypeScript skeleton，同時避免影響既有 V1、避免污染 `web_admin_mvp/` prototype。
...
```text
0.8.0A-12 Create Web Admin App Skeleton
```
...
```text
建立 web_admin_app/
建立 Next.js / TypeScript skeleton
建立基本 layout
...
A12 應仍維持小步前進，不一次導入完整資料模型、登入、後端 API 或正式部署。
